...
// Brute-force algorithm to find the majority element in an array
int brute_force(int arr[], int n) {
    for (int i = 0; i < n; i++) {
        ...
        if (count > n / 2) return arr[i]; // Return the majority if found
    }
...
// Runs the brute force algorithm on generated arrays and measures execution time
void run_tests(void (*generator)(int*, int), const char* label) {
    ...
    for (int t = 0; t < TEST_COUNT; t++) {
        ...
        clock_t start = clock();
        int majority = brute_force(arr, size);
        clock_t end = clock();
        ...
        printf("Majority: %d\n", majority);
        printf("Time: %.20f milliseconds\n\n", time_taken);

        free(arr);
    }

    printf("Average time for %s: %.20f milliseconds\n", label, total_time / TEST_COUNT);
}
