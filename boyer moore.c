...
int boyerMooreMajority(int* arr, int n) {
...
// Runs the test cases for a given generator and prints timing + majority result
void run_tests(void (*generator)(int*, int), const char* label) {
    ...
    for (int t = 0; t < TEST_COUNT; t++) {
        ...
        generator(arr, n); // Generate test input

        printf("Test %d (size = %d):\n", t + 1, n);
        printArray(arr, n); // Print input

        // Measure execution time
        clock_t start = clock();
        int majority = boyerMooreMajority(arr, n);
        clock_t end = clock();
